...
def predict_faces(batch, n_vs: List[int]):
    ...
    for i, mesh in enumerate(batch['x']):
        ...
        if epoch % 1 == 0:
            ...
            mesh.export(file=f'{export_dir}/meshes/epoch_{epoch:04d}/vpos_{name[6:10]}.obj')
...
def save_model(net, optimizer, scheduler, name: str):
    os.makedirs(f'{export_dir}/models/epoch_{epoch:04d}', exist_ok=True)
    torch.save(net.state_dict(), f'{export_dir}/models/epoch_{epoch:04d}/{name}_net.pt')
    torch.save(optimizer.state_dict(), f'{export_dir}/models/epoch_{epoch:04d}/{name}_optimizer.pt')
    torch.save(scheduler.state_dict(), f'{export_dir}/models/epoch_{epoch:04d}/{name}_scheduler.pt')


def save_vert_pos_log(meshes, vert_pos):
    os.makedirs(f'{export_dir}/positions/{epoch:03d}', exist_ok=True)
    for i, mesh in enumerate(meshes):
        with open(f'{export_dir}/positions/{epoch:03d}/{mesh.filename[:-3]}txt', 'w') as f:
            for v in mesh.boundary_loop:
                coords = mesh.vs[v].tolist()
                f.write(f'{coords[0]} {coords[1]} {coords[2]}\n')
            f.write('\n')
            for coords in vert_pos[i].tolist():
                f.write(f'{coords[0]} {coords[1]} {coords[2]}\n')
...
if __name__ == '__main__':
    ...
    train_data, test_data = MRepDataset(opt, True, boundary=True), MRepDataset(opt, False, boundary=True)
    ...
    n_vs_criterion = nn.CrossEntropyLoss()
    n_vs_net = NVertsNet(opt, opt.verts_max, 200).to(device)
    n_vs_optimizer = torch.optim.Adam(n_vs_net.parameters(), lr=opt.lr_n_vs)
    n_vs_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(n_vs_optimizer, patience=30)
    vert_pos_criterion = ChamferDistance(opt, from_mesh=False)
    vert_pos_net = VertPosNet(opt, opt.verts_max, 250)
    vert_pos_optimizer = torch.optim.Adam(vert_pos_net.parameters(), lr=opt.lr_v_pos)
    vert_pos_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(vert_pos_optimizer, patience=30)
    ...
    for epoch in range(opt.start_epoch + 1, opt.epochs + 1):
        ...
        n_vs_epoch_loss = vert_pos_epoch_loss = smooth_epoch_loss = 0

        for i, batch in enumerate(train_loader):

            n_vs_net.train()
            n_vs_optimizer.zero_grad()
            n_vs_pred, n_vs_real = predict_n_verts(batch)
            n_vs_loss = n_vs_criterion(n_vs_pred, n_vs_real)
            n_vs_pred = n_vs_pred.argmax(dim=1)
            n_vs_epoch_loss += n_vs_loss
            n_vs_loss.backward()
            n_vs_optimizer.step()
            n_vs_net.eval()

            vert_pos_net.train()
            vert_pos_optimizer.zero_grad()
            vert_pos_pred, vert_pos_real = predict_vert_positions(batch, n_vs_pred)
            if epoch % 1 == 0:
                save_vert_pos_log(batch['x'], vert_pos_pred)
            vert_pos_loss = vert_pos_criterion(vert_pos_pred, vert_pos_real)
            vert_pos_epoch_loss += vert_pos_loss
            vert_pos_loss.backward()
            vert_pos_optimizer.step()
            vert_pos_net.eval()
        ...
        n_vs_scheduler.step(n_vs_epoch_loss)
        vert_pos_scheduler.step(vert_pos_epoch_loss)
        # smooth_scheduler.step(smooth_epoch_loss)

        with open(f'{export_dir}/n_vs_loss_log.txt', 'a') as f:
            f.write(f'{n_vs_epoch_loss * opt.batch_size / len(train_loader)}\n')
        with open(f'{export_dir}/vert_pos_loss_log.txt', 'a') as f:
            f.write(f'{vert_pos_epoch_loss * opt.batch_size / len(train_loader)}\n')

        if epoch % 1 == 0:
            save_model(n_vs_net, n_vs_optimizer, n_vs_scheduler, 'n_vs')
            save_model(vert_pos_net, vert_pos_optimizer, vert_pos_scheduler, 'v_pos')

        if opt.test_each_epoch:
            n_vs_test_loss = v_pos_test_loss = smooth_test_loss = 0
            for i, data in enumerate(test_loader):
                n_vs_pred, n_vs_real = predict_n_verts(data)
                n_vs_test_loss += n_vs_criterion(n_vs_pred, n_vs_real)
                n_vs_pred = n_vs_pred.argmax(dim=1)

                vert_pos_pred, vert_pos_real = predict_vert_positions(data, n_vs_pred)
                v_pos_test_loss += vert_pos_criterion(vert_pos_pred, vert_pos_real)

                [data['x'][idx].add_verts(pos) for idx, pos in enumerate(vert_pos_pred)]
                meshes_pred, faces_pred, faces_real = predict_faces(data, n_vs_pred)

            with open(f'{export_dir}/n_vs_test_log.txt', 'a') as f:
                f.write(f'{n_vs_test_loss * opt.batch_size / len(test_loader)}\n')
            with open(f'{export_dir}/vert_pos_test_log.txt', 'a') as f:
                f.write(f'{v_pos_test_loss * opt.batch_size / len(test_loader)}\n')

        time_taken = int(time.time() - epoch_start_time)
        print(f'end of epoch {epoch} of {opt.epochs}, time taken: {time_taken} seconds')
        print(f'n_vs_loss: {n_vs_epoch_loss}, v_pos_loss: {vert_pos_epoch_loss}')
        print('learning rates:')
        print(f"n_vs: {n_vs_optimizer.param_groups[0]['lr']}, "
              f"v_pos: {vert_pos_optimizer.param_groups[0]['lr']}")

    num_epochs = opt.epochs - opt.start_epoch
    save_loss_plot(num_epochs, 'n_vs_loss_plot', os.path.join(export_dir, 'n_vs_loss_log.txt'),
                   test_acc_file=os.path.join(export_dir, 'n_vs_test_log.txt'), log_scale=True)
    save_loss_plot(num_epochs, 'vert_pos_loss_plot', os.path.join(export_dir, 'vert_pos_loss_log.txt'),
                   test_acc_file=os.path.join(export_dir, 'vert_pos_test_log.txt'), log_scale=True)
